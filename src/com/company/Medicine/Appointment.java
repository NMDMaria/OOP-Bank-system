...
public class Appointment {
    ...
    public static Comparator<Appointment> AppointmentDate = new Comparator<Appointment>() {

        public int compare(Appointment a, Appointment b) {
           if (a.status == Status.CANCELLED && b.status == Status.CANCELLED)
            return a.date.compareTo(b.date);
           else if (a.status == Status.CANCELLED && b.status != Status.CANCELLED)
               return 1;
           else if (b.status == Status.CANCELLED)
               return 0;
           else return a.date.compareTo(b.date);
        }};
}
